...
@section('content')
                ...
                <div class="flex items-center gap-6">
                    <div class="relative w-20 h-20 flex items-center justify-center">
                        <svg class="w-full h-full transform -rotate-90">
                            <circle cx="40" cy="40" r="35" stroke="#eee" stroke-width="6"
                                fill="transparent" />
                            <circle cx="40" cy="40" r="35" stroke="#E63946" stroke-width="6"
                                fill="transparent" stroke-dasharray="220" stroke-dashoffset="60" />
                        </svg>
                        <i class="fa-solid fa-droplet absolute text-red-500"></i>
                    </div>
                    <div>
                         <span id="donationCount" class="absolute text-lg font-bold text-red-500"></span>
                    </div>
                </div>
            </div>
    ...
    `;
        ...
        $(document).ready(function() {
    $.ajax({
        url: '/api/v1/user/profile', // make sure this route points to your profile function
        type: 'GET',
        success: function(response) {
            // Update the donation count
            $('#donationCount').text(response.donations_count + ' Donations');
            
            // Optional: animate stroke based on donation count (example)
            let maxDonations = 20; // for 100% fill
            let percent = Math.min((response.donations_count / maxDonations) * 220, 220);
            $('circle[stroke="#E63946"]').css('stroke-dashoffset', 220 - percent);
        },
        error: function(err) {
