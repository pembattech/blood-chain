...
$(document).ready(function () {
    ...
    window.fetchRequests = function(page = 1){
        $.ajax({
            ...
            error: function(){
                $('#requests-container').html('<p class="text-gray-500 text-center">Failed to load requests.</p>');
            }
        });
    }
    ...
    function renderRequests(requests){
        ...
        requests.forEach(function(req){
            let urgencyColor = ['HIGH','MEDIUM'].includes(req.urgency?.toUpperCase()) ? 'red' : 'blue';
            ...
            // Blood type color
            let bloodColorClass = bloodTypeColors[req.blood_type_needed] || 'bg-gray-100 text-gray-600';
            ...
            <div class="bg-white border-2 border-transparent request-card rounded-xl p-6 shadow-sm flex items-center justify-between border-l-8 border-l-${urgencyColor}-500 hover:shadow-lg transition">
                ...
                <div class="flex items-center gap-4">
                    <button class="text-blue-600 font-bold text-sm hover:underline">View Details</button>
    ...
    function renderPagination(meta){
        ...
        for(let i=1;i<=meta.last_page;i++){
            ...
            let button = `<button onclick="fetchRequests(${i})" class="w-10 h-10 flex items-center justify-center rounded-lg ${activeClass}">${i}</button>`;
